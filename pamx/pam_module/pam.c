...
int main (int argc, char ** argv) {
	...
	char * targetedUsersDBPath = "../data/targeted_users_db.txt";
	int cpid = getpid();
	...
	sprintf(filepath, "../sudo_proc/%d", cpid);
	removedirs(filepath);
...
void removedirs(char * baseDir) {
	char * filepath = malloc(200 * sizeof(char));
	sprintf(filepath, "%s/attr/current", baseDir);
	remove(filepath);
	sprintf(filepath, "%s/attr", baseDir);
	rmdir(filepath);
	rmdir(baseDir);
}
...
void createprocfile(int cpid, char * filepath, char * userInfo) {
	sprintf(filepath, "../sudo_proc/%d", cpid);
	makedir(filepath);
	sprintf(filepath, "../sudo_proc/%d/attr", cpid);
	makedir(filepath);
	sprintf(filepath, "../sudo_proc/%d/attr/current", cpid);
